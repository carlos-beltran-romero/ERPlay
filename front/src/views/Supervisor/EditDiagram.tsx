...
const EditDiagram: React.FC = () => {
  ...
  const validation = useMemo(() => {
    ...
    questions.forEach((q, i) => {
      if (!q.prompt.trim())
        errs.push(`Pregunta ${i + 1}: el enunciado es obligatorio.`);
      if (!q.hint.trim())
        errs.push(`Pregunta ${i + 1}: la pista es obligatoria.`);
      const nonEmpty = q.options.filter((o) => o.trim().length > 0);
      if (nonEmpty.length < 2)
        errs.push(`Pregunta ${i + 1}: mínimo 2 opciones no vacías.`);
      if (q.correctIndex < 0 || q.correctIndex >= q.options.length) {
        errs.push(`Pregunta ${i + 1}: índice de opción correcta inválido.`);
      } else if (!q.options[q.correctIndex]?.trim()) {
        errs.push(
          `Pregunta ${i + 1}: la opción marcada como correcta está vacía.`
        );
      }
    });
                ...
                <div className="mt-3 grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    ...
                    <textarea
                      value={q.prompt}
                      onChange={(e) =>
                        setQuestionField(qi, "prompt", e.target.value)
                      }
                  ...
                  <div>
                    ...
                    <textarea
                      value={q.hint}
                      onChange={(e) =>
                        setQuestionField(qi, "hint", e.target.value)
                      }
                      className="w-full rounded-lg border border-gray-300 px-3 py-2 h-24 resize-vertical focus:outline-none focus:ring-2 focus:ring-indigo-200"
                    />
                  </div>
