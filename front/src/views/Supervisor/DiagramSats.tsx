...
import {
  getDiagramStats,
  type DiagramStatsResponse,
} from "../../services/diagramStats";
  ...
  <Card>
    <CardBody className="flex items-center gap-3">
      ...
      <div className="flex flex-col">
        ...
        {hint ? (
          <span className="text-[11px] text-gray-500 mt-0.5">{hint}</span>
        ) : null}
      </div>
    </CardBody>
  </Card>
...
/* ---------- Helpers ---------- */
const pct = (n?: number) =>
  `${Math.round(Math.max(0, Math.min(100, Number(n ?? 0))))}%`;
const fmt10 = (n?: number) => (Number.isFinite(n) ? Number(n).toFixed(1) : "—");
const fmtSec1 = (n?: number) =>
  Number.isFinite(n) ? `${Number(n).toFixed(1)} s` : "—";
...
const Pill: React.FC<{
  ...
  <span
    title={title}
    className={`inline-block rounded-lg px-2 py-0.5 text-xs font-medium ${tone}`}
  >
    {children}
  </span>
);
...
const StackedBar: React.FC<{ parts: { label: string; value: number }[] }> = ({
  parts,
}) => {
      ...
      {parts.map((p, i) => (
        <div
          ...
          style={{
            ...
            background: ["#3b82f6", "#10b981", "#f59e0b", "#ef4444", "#6366f1"][
              i % 5
            ],
          }}
...
const SectionInfoButton: React.FC<{
  onClick: () => void;
  open: boolean;
  title?: string;
  className?: string;
}> = ({ onClick, open, title = "Información", className }) => (
  <button
    onClick={onClick}
...
const SectionInfoButtonSmall: React.FC<{
  onClick: () => void;
  open: boolean;
  title?: string;
}> = ({ onClick, open, title = "Información" }) => (
  <button
    onClick={onClick}
...
const ExplainPanel: React.FC<{ children: React.ReactNode }> = ({
  children,
}) => (
...
const ShowMoreLess: React.FC<{ text: string; limit?: number }> = ({
  text,
  limit = 120,
}) => {
...
const DiagramStats: React.FC = () => {
  ...
  useEffect(() => {
    setDriftVisible(DRIFT_PAGE);
  }, [id, from, to, stats?.drift?.length]);
  ...
  const krBadge = (() => {
    if (reliability == null)
      return <Pill tone="bg-gray-100 text-gray-700">—</Pill>;
    ...
    return (
      <Pill tone={tone} title="0–1 (más alto = más consistente)">
        {v.toFixed(2)}
      </Pill>
    );
  })();
      ...
      <div className="mx-auto w-full max-w-7xl p-4 sm:p-6 space-y-6 sm:space-y-8">
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 xl:grid-cols-4 gap-4">
          <KPI
            ...
            label="Nota examen (media)"
            value={`${fmt10(k.examScoreAvg10)}/10`}
          />
          <KPI
            icon={<Activity className="h-5 w-5" />}
            label="Acierto en práctica"
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 xl:grid-cols-3 gap-4">
            ...
            <CardBody>
              <div className="flex items-start justify-between gap-2">
                <div className="flex items-center gap-2">
                  <span className="text-xs uppercase tracking-wide text-gray-500">
                    Examen consistente (0–1)
                  </span>
                  <SectionInfoButton
                    onClick={() => setOpenReliabilityInfo((o) => !o)}
                    open={openReliabilityInfo}
                  />
                </div>
              ...
              <div className="text-[11px] text-gray-500">
                Más alto = resultados más estables
              </div>

              {openReliabilityInfo && (
                <ExplainPanel>
                  Indica si el examen evalúa <b>siempre lo mismo</b> (estable).
                  Va de 0 a 1.
                  <div className="mt-1">
                    Guía rápida: <b>0.80–1.00</b> muy estable · <b>0.70–0.79</b>{" "}
                    aceptable · <b>&lt; 0.70</b> revisar preguntas.
                  </div>
                </ExplainPanel>
              )}
            </CardBody>
          ...
          <Card>
            <CardBody>
              ...
              <div className="text-[11px] text-gray-500">
                Porcentaje de preguntas donde se pidió pista
              </div>
            </CardBody>
          </Card>
        </div>
        ...
        <Card>
          <CardBody>
            ...
            {openDiscrInfo && (
              <ExplainPanel>
                <b>Diferencia de niveles (−1..1)</b>: Mide si la pregunta la
                aciertan los que sacan buena nota y la fallan los que sacan mala
                nota.<br></br> <b>≥ 0.30</b> Buena pregunta ; <b>0.10–0.29</b>{" "}
                Separa poco; <b>≈ 0</b> No distingue; <b> Negativo</b> La
                aciertan más los que peor nota total tienen → revisar
              </ExplainPanel>
            )}
                ...
                <table className="w-full text-sm">
                    ...
                    <tr className="text-left text-gray-500">
                      ...
                      <th className="py-2">
                        <span className="inline-flex items-center gap-1">
                          Diferencia niveles
                          <SectionInfoButtonSmall
                            onClick={() => setOpenDiscrInfo((o) => !o)}
                            open={openDiscrInfo}
                          />
                        </span>
                      </th>
                  ...
                  <tbody>
                    {stats.itemQuality.map((q) => {
                      ...
                      const discrScaled = Math.max(
                        -1,
                        Math.min(1, q.discrPointBiserial)
                      );
                      const discrPct = ((discrScaled + 1) / 2) * 100;
                      const discrTone = classForScore(discrPct, true);
                      const timeTone = classForScore(
                        Math.min(100, (q.medianTimeSec ?? 0) * 5),
                        false
                      );
                      ...
                      return (
                        <tr key={q.questionId} className="border-t">
                          ...
                          <td className="py-2">
                            <Pill tone={diffTone}>{pct(q.pCorrectPct)}</Pill>
                          </td>
                          <td className="py-2">
                            <Pill
                              tone={discrTone}
                              title="−1..1 (más alto = separa mejor)"
                            >
                              {q.discrPointBiserial.toFixed(2)}
                            </Pill>
                          </td>
                          <td className="py-2">
                            <Pill tone={timeTone}>
                              {fmtSec1(q.medianTimeSec ?? undefined)}
                            </Pill>
                          </td>
                          <td className="py-2">
                            <Pill tone={claimTone}>{pct(q.claimRatePct)}</Pill>
                          </td>
        ...
        <Card>
          <CardBody>
            <div className="flex items-center justify-between mb-2">
              ...
              <SectionInfoButton
                onClick={() => setOpenDistractorsInfo((o) => !o)}
                open={openDistractorsInfo}
              />
            </div>

            {openDistractorsInfo && (
              <ExplainPanel>
                Muestra qué opciones equivocadas atraen más clics. Sirve para
                detectar errores “atractivos” y mejorar enunciados u opciones
                así como ver los fallos comunes.
              </ExplainPanel>
            )}
            ...
            {!stats.distractors?.length ? (
              ...
              <div className="space-y-3">
                {Object.entries(groupBy(stats.distractors, (d) => d.questionId))
                  .slice(0, 6)
                  .map(([qid, arr]) => {
                    const title =
                      stats.itemQuality?.find((i) => i.questionId === qid)
                        ?.title ?? `Pregunta ${qid}`;
        ...
        <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
          <Card>
            <CardBody>
              ...
              <div className="text-sm text-gray-600 mb-3">
                ¿Cuántos intentos suelen necesitar para ≥ 8/10?
              </div>

              {stats.learningCurves ? (
                stats.learningCurves.attemptsToMasteryP50 != null ? (
                  <div className="text-2xl font-semibold">
                    {stats.learningCurves.attemptsToMasteryP50} intentos
                    (mediana)
                  </div>
                ) : (
                  <div className="text-sm text-gray-500">
          ...
          <Card>
            <CardBody>
              <div className="text-lg font-semibold mb-1">
                Mejora de learning a examen
              </div>
              <div className="text-sm text-gray-600 mb-3">
                Diferencia media de nota. <br></br>Debe estar seleccionada un
                rango de fecha.
              </div>

              {stats.learningCurves ? (
                <div className="flex items-center gap-2 text-2xl font-semibold">
                  {(stats.learningCurves.deltaPracticeToExamAvgPts ?? 0) >=
                  0 ? (
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-4">
                  ...
                  {stats.hotspots.map((h) => (
                    ...
                    >
                      ...
                      <div className="mt-1 grid grid-cols-2 sm:grid-cols-4 gap-2 text-xs">
                        ...
                        <div>
                          <span className="text-gray-500">Tiempo mediano:</span>{" "}
                          <span className="font-semibold">
                            {fmtSec1(h.medianTimeSec ?? undefined)}
                          </span>
                        </div>
                        <div>
          ...
          <Card>
            <CardBody>
              ...
              {openRiskInfo && (
                <ExplainPanel>
                  Consideramos “en riesgo” a quien en su <b>último examen</b> de
                  este diagrama ha sacado <b>≤ 5/10</b>. Útil para priorizar
                  refuerzos. Se muestra su número de intentos y la fecha del
                  último intento.
                </ExplainPanel>
              )}
              ...
              {!stats.riskStudents.length ? (
                <div className="text-sm text-gray-500">
                  Sin alumnos en riesgo en el rango.
                </div>
              ) : (
                <div className="overflow-x-auto">
                  <table className="w-full text-sm">
        ...
        <Card>
          <CardBody>
            <div className="flex items-center justify-between">
              <div className="text-lg font-semibold">
                Deriva de preguntas (cambio en el rango)
              </div>
              <SectionInfoButton
                onClick={() => setOpenDriftInfo((o) => !o)}
                open={openDriftInfo}
              />
            </div>
            ...
            {openDriftInfo && (
              <ExplainPanel>
                Compara la <b>primera mitad</b> vs la <b>segunda mitad</b> del
                periodo elegido. “Cambio aciertos” indica si ahora se acierta
