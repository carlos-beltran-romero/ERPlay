...
export class UsersService {
  ...
  private async sendWelcomeCredentialsEmail(user: SafeUser, password: string) {
    ...
    `;
    const body = `
      <p style="margin:0 0 16px 0;">Hola ${escapeHtml(
    ...
    try {
      ...
      console.error(
        `No se pudo enviar el correo de credenciales a ${user.email}:`,
        error
      );
    }
  }
  ...
  async sendPasswordUpdatedEmail(user: SafeUser, password: string) {
    ...
    try {
      ...
      console.error(
        `No se pudo enviar el correo de actualización a ${user.email}:`,
        error
      );
    }
  }
