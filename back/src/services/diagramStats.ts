...
export async function getDiagramStatsService(diagramId: string, range?: Range): Promise<DiagramStatsResponse> {
  ...
  const learningAccuracyPct = learningSessions.length
    ? avg(learningSessions.map(s => ratioPct(s.correctCount, s.totalQuestions)))
    : 0;

  const masteryRatePct = examSessions.length
    ? (examSessions.filter(s => num(s.score) >= 8).length * 100) / examSessions.length
    : 0;

  const atRiskRatePct = examSessions.length
    ? (examSessions.filter(s => num(s.score) <= 5).length * 100) / examSessions.length
    : 0;
  ...
  const errorAgg = Object.entries(byQuestion).map(([qid, rs]) => {
    ...
    return {
      ...
      title: (rs.find(r => r.question?.prompt)?.question?.prompt) ?? `Pregunta ${qid}`,
    };
  });
  const top5 = [...errorAgg].sort((a, b) => b.errorRatePct - a.errorRatePct).slice(0, 5);
  const totalErrorsAll = errorAgg.reduce((s, r) => s + r.errors, 0);
  const errorConcentrationTop5Pct = totalErrorsAll ? pctNum(top5.reduce((s, r) => s + r.errors, 0), totalErrorsAll) : 0;
  ...
  for (const uid of Object.keys(examsByUser)) {
    ...
    const practiceAvg10 = avg(leArr.map(s => {
  ...
  const trends: TrendPoint[] = Object.entries(byDay)
    .map(([day, arr]) => {
      ...
      return {
        date: day,
        examScorePct: exams.length ? avg(exams.map(s => num(s.score) * 10)) : null,
        learningAccuracyPct: learns.length ? avg(learns.map(s => ratioPct(s.correctCount, s.totalQuestions))) : null,
      };
    })
  ...
  for (const s of examSessions) {
    const uid = s.user.id;
    const times = (s.results || []).map(r => num(r.timeSpentSeconds)).filter(isFiniteNum);
  ...
  const scatterSpeedVsAccuracy: ScatterPoint[] = Object.keys({ ...learnAccByUser, ...examTimeByUser })
    .map(uid => ({
      studentId: uid,
  ...
  for (const [qid, rs] of Object.entries(byQuestion)) {
    ...
    titleByQ[qid] = (rs.find(r => r.question?.prompt)?.question?.prompt) ?? `Pregunta ${qid}`;
  ...
  if (range?.from && range?.to) {
    claimQB.andWhere('c.createdAt BETWEEN :from AND :to', { from: new Date(range.from), to: new Date(range.to) });
  } else if (range?.from) {
  ...
  const claimRows = await claimQB
    ...
    .groupBy('q.id')
    .getRawMany<{ qid: string; approved: string | number; total: string | number }>();
  ...
  const itemQuality: ItemQuality[] = Object.keys({ ...attemptsByQ, ...discData })
    .map(qid => {
      ...
      const rpb = pointBiserial(discRows.map(d => d.correct01), discRows.map(d => d.totalExcl));
  ...
  const scoreValues = Object.values(userExamScores).filter(isFiniteNum).sort((a, b) => a - b);
  const q25 = scoreValues.length ? quantile(scoreValues, 0.25) : 0;
  const q75 = scoreValues.length ? quantile(scoreValues, 0.75) : 0;

  for (const [qid, rs] of Object.entries(byQuestion)) {
    const sel = rs.filter(r => r.selectedIndex != null && Array.isArray(r.optionsSnapshot));
    ...
    for (const [txt, n] of mapGlobal) {
      distractors.push({
        ...
        chosenPctLowQuartile: mapLow.has(txt) ? round1(pctNum(mapLow.get(txt)!, Math.max(1, sumMap(mapLow)))) : undefined,
        chosenPctHighQuartile: mapHigh.has(txt) ? round1(pctNum(mapHigh.get(txt)!, Math.max(1, sumMap(mapHigh)))) : undefined,
      });
    }
  }
  ...
  for (const [uid, arr] of Object.entries(examsByUser)) {
    ...
    for (const s of ordered) {
      count += 1;
      if (num(s.score) >= 8) { reached = true; break; }
    }
    if (reached) attemptsUntilMastery.push(count);
  }
  const attemptsToMasteryP50 = attemptsUntilMastery.length ? Math.round(quantile(attemptsUntilMastery, 0.5)) : null;
  ...
  if (modalK > 1) {
    const exByQ = groupBy(examResults.filter(r => r.question?.id), r => r.question!.id);
    ...
    const sum_pq = topQids.reduce((s, qid) => {
      const p = piByQ[qid] ?? 0; const q = 1 - p;
      return s + (p * q);
    }, 0);

    const totalScores = examSessions
      .map(s => {
        const rs = (s.results || []).filter(r => r.question?.id && topQids.includes(r.question!.id));
  ...
  const byQFirst = groupBy(firstHalf.flatMap(s => s.results || []).filter(r => r.question?.id), r => r.question!.id);
  const byQSecond = groupBy(secondHalf.flatMap(s => s.results || []).filter(r => r.question?.id), r => r.question!.id);

  const drift: DriftItem[] = Object.keys({ ...byQFirst, ...byQSecond }).map(qid => {
    const r1 = byQFirst[qid] || [];
    const r2 = byQSecond[qid] || [];
    const p1 = r1.length ? pctNum(r1.filter(r => r.isCorrect === true).length, r1.length) : 0;
    const p2 = r2.length ? pctNum(r2.filter(r => r.isCorrect === true).length, r2.length) : 0;
    const t1 = median(r1.map(r => num(r.timeSpentSeconds)));
    const t2 = median(r2.map(r => num(r.timeSpentSeconds)));
    return {
      questionId: qid,
      title: (r1[0]?.question?.prompt || r2[0]?.question?.prompt) ?? `Pregunta ${qid}`,
      deltaPCorrectPct: round1(p2 - p1),
      deltaMedianTimeSec: t1 != null && t2 != null ? round2(t2 - t1) : (t2 ?? t1 ?? null),
    };
...
function sumMap(m: Map<string, number>): number { let s = 0; for (const v of m.values()) s += v; return s; }
...
function mostCommonWrong(rs: TestResult[]): string | null {
  ...
  for (const r of rs) {
    if (r.isCorrect === false && r.selectedIndex != null && Array.isArray(r.optionsSnapshot)) {
      const txt = r.optionsSnapshot[r.selectedIndex];
      if (!txt) continue;
